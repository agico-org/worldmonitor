...
import {
  ...
  allButtons,
  buttonsForPlatform,
} from "@/components/DownloadBanner";
...
export class EventHandlerManager implements AppModule {
  ...
  private boundDesktopExternalLinkHandler: ((e: MouseEvent) => void) | null =
    null;
  ...
  private boundDropdownKeydownHandler: ((e: KeyboardEvent) => void) | null =
    null;
  ...
  private boundMapWidthResizeMoveHandler: ((e: MouseEvent) => void) | null =
    null;
  private boundMapWidthEndResizeHandler: (() => void) | null = null;
  private boundMapFullscreenEscHandler: ((e: KeyboardEvent) => void) | null =
    null;
  ...
  private toggleTvMode(): void {
    ...
    document
      .getElementById("tvModeBtn")
      ?.classList.toggle("active", this.ctx.tvMode.active);
  }
  ...
  destroy(): void {
    ...
    if (this.boundDesktopExternalLinkHandler) {
      document.removeEventListener(
        "click",
        this.boundDesktopExternalLinkHandler,
        true,
      );
    ...
    if (this.boundMapWidthResizeMoveHandler) {
      document.removeEventListener(
        "mousemove",
        this.boundMapWidthResizeMoveHandler,
      );
    ...
    if (this.boundMapFullscreenEscHandler) {
      document.removeEventListener(
        "keydown",
        this.boundMapFullscreenEscHandler,
      );
  ...
  private setupEventListeners(): void {
    ...
    document
      .getElementById("searchMobileFab")
      ?.addEventListener("click", () => {
    ...
    this.boundStorageHandler = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.panels && e.newValue) {
        try {
          this.ctx.panelSettings = JSON.parse(e.newValue) as Record<
            string,
            PanelConfig
          >;
          this.applyPanelSettings();
    ...
    this.boundWidgetModifyHandler = ((e: CustomEvent<{ widgetId: string }>) => {
      ...
      openWidgetChatModal({
        ...
        onComplete: (updated) => {
          saveWidget(updated);
          (
            this.ctx.panels[updated.id] as CustomWidgetPanel | undefined
          )?.updateSpec(updated);
        },
      });
    }) as EventListener;
    ...
    this.ctx.container.addEventListener("wm:mcp-configure", ((
      ...
      openMcpConnectModal({
        ...
        onComplete: (updated) => {
          ...
          (this.ctx.panels[updated.id] as McpDataPanel | undefined)?.updateSpec(
            updated,
          );
        },
      });
  ...
  private setupMobileMenu(): void {
    ...
    const isLocalDev =
      location.hostname === "localhost" || location.hostname === "127.0.0.1";
    menu
      .querySelectorAll<HTMLButtonElement>(".mobile-menu-variant")
      .forEach((btn) => {
    ...
    document
      .getElementById("mobileMenuSettings")
      ?.addEventListener("click", () => {
        this.closeMobileMenu();
    ...
    document
      .getElementById("mobileMenuTheme")
      ?.addEventListener("click", () => {
        this.closeMobileMenu();
    ...
    sheet
      ?.querySelectorAll<HTMLButtonElement>(".region-sheet-option")
      .forEach((opt) => {
  ...
  private closeMobileMenu(): void {
    ...
    const sheetOpen = document
      .getElementById("regionBottomSheet")
  ...
  getShareUrl(): string | null {
    ...
    return buildMapUrl(baseUrl, {
      ...
      country: isCountryVisible
        ? (briefPage?.getCode() ?? undefined)
        : undefined,
      expanded:
        isCountryVisible && briefPage?.getIsMaximized?.() ? true : undefined,
    });
  }
  ...
  private initDownloadDropdown(): void {
    ...
    const renderDropdown = () => {
      ...
      dropdown
        .querySelectorAll<HTMLAnchorElement>(".dl-dd-btn")
        .forEach((a) => {
          a.addEventListener("click", (e) => {
            e.preventDefault();
            const plat =
              new URL(a.href, location.origin).searchParams.get("platform") ||
              "unknown";
      ...
      const othersEl = dropdown.querySelector(
        "#dlDdOthers",
      ) as HTMLElement | null;
      if (toggle && othersEl) {
  ...
  private initFooterDownload(): void {
    ...
    a.addEventListener("click", (e) => {
      e.preventDefault();
      const plat =
        new URL(btn.href, location.origin).searchParams.get("platform") ||
        "unknown";
      trackDownloadClicked(plat);
  ...
  setupExportPanel(): void {
    ...
    this.ctx.exportPanel = new ExportPanel(() => {
      ...
      return {
        ...
        newsClusters:
          this.ctx.latestClusters.length > 0
      ...
      toggleSource: (name: string) => {
        ...
        if (reenabling && !isProUser()) {
          ...
          if (currentlyEnabled + 1 > FREE_MAX_SOURCES) {
            this.showToast(
              t("modals.settingsWindow.freeSourceLimit", {
                max: String(FREE_MAX_SOURCES),
              }),
            );
            return;
          }
        ...
        saveToStorage(
          ...
          Array.from(this.ctx.disabledSources),
        );
      },
  ...
  setupMapLayerHandlers(): void {
    this.ctx.map?.setOnLayerChange((layer, enabled, source) => {
      ...
      if (layer === "flights") {
        const airlineIntel = this.ctx.panels["airline-intel"] as
          | AirlineIntelPanel
          | undefined;
  ...
  setupMapResize(): void {
    ...
    const getMaxHeight = () => {
      if (window.innerWidth < 1600)
        return Math.max(getMinHeight(), window.innerHeight - 150);
    ...
    const getTarget = () =>
      window.innerWidth >= 1600 ? mapContainer : mapSection;
    ...
    resizeHandle.addEventListener("dblclick", () => {
      ...
      const finalHeight = Math.max(
        getMinHeight(),
        Math.min(targetHeight, getMaxHeight()),
      );
    ...
    this.boundMapResizeMoveHandler = (e: MouseEvent) => {
      ...
      const newHeight = Math.max(
        getMinHeight(),
        Math.min(startHeight + deltaY, getMaxHeight()),
      );
  ...
  setupMapWidthResize(): void {
    ...
    widthHandle.addEventListener("mousedown", (e) => {
      ...
      const raw =
        mainContent.style.getPropertyValue("--map-col-width") || "60%";
    ...
    this.boundMapWidthResizeMoveHandler = (e: MouseEvent) => {
      ...
      const newPct = Math.max(
        25,
        Math.min(75, ((startColPx + delta) / startTotalWidth) * 100),
      );
  ...
  private setupMapDimensionToggle(): void {
    ...
    if (!toggle) return;
    toggle
      .querySelectorAll<HTMLButtonElement>(".map-dim-btn")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
